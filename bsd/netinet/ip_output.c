 ...
 */
int
ip_output(struct mbuf *m0, struct mbuf *opt, struct route *ro, int flags, struct ip_moptions *imo)
{
	struct ip *ip, *mhip;
	struct ifnet *ifp;
	struct mbuf *m = m0;
	...
	struct mbuf *m;
{
	...
	if (imo == NULL) {
		 ...
		 */
		imo = (struct ip_moptions*)M_malloc(sizeof(*imo), M_IPMOPTS,
		    M_WAITOK);
