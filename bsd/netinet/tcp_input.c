...
static void tcp_dooptions
		(struct tcpcb *, u_char *, int, struct tcpiphdr *, struct tcpopt *);
/*
...
#define GETTCP(m)       ((struct tcpiphdr *)m->m_pkthdr.header)
	...
	while (q)
	{
		int i = (ti->ti_seq + ti->ti_len) - GETTCP(q)->ti_seq;
		if (i <= 0)
			break;
...
void tcp_input(struct mbuf *m, int iphlen)
{
			...
			&& tp->snd_nxt == tp->snd_max)
	{
		 ...
		 */
		if ((to.to_flag & TOF_TS) != 0 && SEQ_LEQ(ti->ti_seq, tp->last_ack_sent))
		{
	 ...
	 */
	if ((to.to_flag & TOF_TS) && (tiflags & TH_RST) == 0 && tp->ts_recent &&
	 ...
	 */
	if ((to.to_flag & TOF_TS) && SEQ_LEQ(ti->ti_seq, tp->last_ack_sent) &&
...
static void tcp_dooptions(struct tcpcb *tp, u_char *cp, int cnt,
		struct tcpiphdr *ti, struct tcpopt *to)
	...
	for (; cnt > 0; cnt -= optlen, cp += optlen)
	{
		...
		case TCPOPT_CC:
			if (optlen != TCPOLEN_CC)
				continue;
			to->to_flag |= TOF_CC;
			bcopy((char *) cp + 2, (char *) &to->to_cc, sizeof(to->to_cc));
			NTOHL(to->to_cc);
			/*
		...
		case TCPOPT_CCECHO:
			...
			bcopy((char *) cp + 2, (char *) &to->to_ccecho,
					sizeof(to->to_ccecho));
			NTOHL(to->to_ccecho);
			break;
		}
...
int tcp_mss(struct tcpcb *tp, u_int offer)
{
	...
	if (tp->t_srtt == 0 && (rtt = rt->rt_rmx.rmx_rtt))
	{
		...
		TCPT_RANGESET(tp->t_rxtcur, ((tp->t_srtt >> 2) + tp->t_rttvar) >> 1,
