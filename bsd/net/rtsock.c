...
static int
rts_attach(struct socket *so, int proto)
{
        struct rawcb *rp;
        int s, error;
...
static int
rts_bind(struct socket *so, struct mbuf *nam)
{
        int s, error;
        error = raw_usrreqs.pru_bind(so, nam); /* xxx just EINVAL */
        return error;
}

static int
rts_connect(struct socket *so, struct mbuf *nam)
{
        int s, error;
        error = raw_usrreqs.pru_connect(so, nam); /* XXX just EINVAL */
        return error;
}
...
{
        int s, error;
        error = raw_usrreqs.pru_send(so, flags, m, nam, control);
        return error;
}
...
struct pr_usrreqs route_usrreqs = {
		.pru_abort =            rts_abort,
	...
	if (cp == 0 && w != NULL && !second_time) {
		...
		if (rw->w_needed <= 0 && rw->w_where) {
			if (rw->w_tmemsize < len) {
				if (rw->w_tmem)
					M_free(rw->w_tmem, M_RTABLE);
				if (rw->w_tmem = (caddr_t)
						M_malloc(len, M_RTABLE, M_NOWAIT))
					rw->w_tmemsize = len;
			}
			if (rw->w_tmem) {
				cp = rw->w_tmem;
